...
def calculateConfusionMatrix(model, data_test_gen):
    ...
    for i in range(num_batches):
        images, labels = data_test_gen.next()
        for label in labels:
...
def trainingModel():
    ...
    # Mengubah semua data menjadi parameter yang diingingkan dari datagen
...
def displaySidebarResults(model):
    ...
    # create sidebar
    st.sidebar.markdown("<h1 style='text-align:center'>Hasil Confusion Matrix</h1>", unsafe_allow_html=True)
    st.set_option('deprecation.showPyplotGlobalUse', False) # Agar warning tidak keluar
    st.sidebar.pyplot(plt.show())
    st.sidebar.markdown("<h1 style='text-align:center'>Hasil Akurasi Per Kelas</h1>", unsafe_allow_html=True)
    ...
    if btn_predict_image:
        if image_predict:
            ...
            for probability, label in zip(result[0], list(st.session_state.label_data.values())):
                st.write("Kelas: %s - Probabilitas : %.3f " % (label, probability*100))
        else:
